...
func (t *Task) listDBAndCollectionFromAPI(ctx context.Context) ([]string, []collection, error) {
	...
	if t.grpc.SupportMultiDatabase() {
		...
		dbNames = append(dbNames, _defaultDBName)
		resp, err := t.grpc.ListCollections(ctx, _defaultDBName)
		if err != nil {
			return nil, nil, fmt.Errorf("backup: list collections for db %s: %w", _defaultDBName, err)
		}
		for _, coll := range resp.CollectionNames {
			t.logger.Debug("need backup collection", zap.String("db", _defaultDBName), zap.String("collection", coll))
			collections = append(collections, collection{dbName: _defaultDBName, collName: coll})
		}
	}
