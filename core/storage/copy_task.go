...
func (rp *serverCopier) copy(ctx context.Context, copyAttr CopyAttr) error {
	rp.logger.Debug("copy object", zap.String("src", copyAttr.Src.Key), zap.String("dest", copyAttr.DestKey))
	obj, err := rp.src.GetObject(ctx, copyAttr.Src.Key)
	if err != nil {
		return err
	}
	...
	if err := rp.dest.UploadObject(ctx, i); err != nil {
