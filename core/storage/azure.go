...
func NewAzureClient(cfg Config) (*AzureClient, error) {
	if cfg.UseIAM {
		...
		if len(cfg.Token) != 0 {
			// use SAS Token
			sasURL := fmt.Sprintf("%s?%s", cfg.Endpoint, cfg.Token)
			cli, err := azblob.NewClientWithNoCredential(sasURL, nil)
			if err != nil {
				return nil, fmt.Errorf("storage: new azure client with SAS token %w", err)
			}
			sasCli, err := service.NewClientWithNoCredential(sasURL, nil)
			if err != nil {
				return nil, fmt.Errorf("storage: new azure service client with SAS token %w", err)
			}
			return &AzureClient{cfg: cfg, cli: cli, sasCli: sasCli}, nil
		}
	}

	return nil, fmt.Errorf("storage: not use IAM and not provide secret key")
}
