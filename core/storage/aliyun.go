...
func (a *aliCredProvider) Retrieve() (minioCred.Value, error) {
	...
	if err != nil {
		return minioCred.Value{}, fmt.Errorf("storage: get access key id from aliyun credential: %w", err)
	}
	sk, err := a.cred.GetAccessKeySecret()
	if err != nil {
		return minioCred.Value{}, fmt.Errorf("get access key secret from aliyun credential %w", err)
	}
	securityToken, err := a.cred.GetSecurityToken()
	if err != nil {
		return minioCred.Value{}, fmt.Errorf("get security token from aliyun credential %w", err)
	}

	a.akCache = tea.StringValue(ak)
	return minioCred.Value{
		AccessKeyID:     tea.StringValue(ak),
		SecretAccessKey: tea.StringValue(sk),
		SessionToken:    tea.StringValue(securityToken),
	}, nil
}
