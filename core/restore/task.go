...
func (t *Task) prepareDB(ctx context.Context) error {
	...
	var dbsNeedToRestores []string
	for _, collTask := range t.task.GetCollectionRestoreTasks() {
		dbsNeedToRestores = append(dbsNeedToRestores, collTask.GetTargetDbName())
	}
	...
	for _, db := range dbNotInTarget {
		...
		t.logger.Info("create database", zap.String("database", db))
	}
...
func (t *Task) runCollTask(ctx context.Context) error {
	...
	t.logger.Info("finish restore all collections",
		...
		zap.Duration("duration", time.Duration(t.task.GetEndTime()-t.task.GetStartTime())*time.Second))
	return nil
}
