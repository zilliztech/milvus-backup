...
func TestL0SegmentBatches(t *testing.T) {
	...
	segs := make([]*backuppb.SegmentBackupInfo, 0, _bulkInsertRestfulAPIChunkSize*4)
	for i := 0; i < _bulkInsertRestfulAPIChunkSize*4; i++ {
	...
	for _, b := range batches {
		require.Len(t, b.partitionDirs, _bulkInsertRestfulAPIChunkSize)
		for _, dir := range b.partitionDirs {
			require.Empty(t, dir.insertLogDir)
			expectDir := fmt.Sprintf("binlogs/delta_log/1/1/%s/", dir.deltaLogDir)
			require.NotEmpty(t, expectDir, dir.deltaLogDir)
		}
	}
}
