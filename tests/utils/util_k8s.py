...
def wait_pods_ready(namespace, label_selector, expected_num=None, timeout=360):
    ...
    try:
        ...
        if all_pos_ready_flag:
            log.info(
                f"all pods in namespace {namespace} with label {label_selector} are ready"
            )
        else:
            log.info(
                f"timeout for waiting all pods in namespace {namespace} with label {label_selector} ready"
            )
...
def get_milvus_instance_name(
    namespace, host="127.0.0.1", port="19530", milvus_sys=None
):
    """
...
def get_milvus_deploy_tool(namespace, milvus_sys):
    ...
    if (
        ...
        and api_response.metadata.labels["app.kubernetes.io/managed-by"]
        == "milvus-operator"
    ):
...
def export_pod_logs(namespace, label_selector, release_name=None):
    ...
    pod_log_path = (
        ...
        else f"/tmp/milvus_logs/{release_name}"
    )
    ...
    try:
        ...
        for item in items:
            ...
            logs = api_instance.read_namespaced_pod_log(
                name=pod, namespace=namespace, async_req=True
            )
...
def get_metrics_querynode_sq_req_count():
    ...
    if response.status_code == 200:
        ...
        raise Exception(
            -1, f"Failed to get metrics with status code {response.status_code}"
        )
