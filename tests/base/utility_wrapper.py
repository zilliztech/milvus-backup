...
class ApiUtilityWrapper:
    ...
    def get_bulk_insert_state(
        ...
        **kwargs,
        ...
        func_name = sys._getframe().f_code.co_name
        res, is_succ = api_request(
            [self.ut.get_bulk_insert_state, task_id, timeout, using], **kwargs
        )
        check_result = ResponseChecker(
            res,
            func_name,
    ...
    def list_bulk_insert_tasks(
        ...
        **kwargs,
        ...
        func_name = sys._getframe().f_code.co_name
        res, is_succ = api_request(
            [self.ut.list_bulk_insert_tasks, limit, collection_name, timeout, using],
            **kwargs,
        )
        check_result = ResponseChecker(
            res,
    ...
    def wait_for_bulk_insert_tasks_completed(
        self,
        task_ids,
        target_state=BulkInsertState.ImportCompleted,
        timeout=None,
        using="default",
        **kwargs,
    ):
        ...
        while (
            len(tasks_state_distribution["success"])
            + len(tasks_state_distribution["failed"])
            < len(task_ids)
            ...
            for task_id in task_ids:
                if (
                    task_id in tasks_state_distribution["success"]
                    or task_id in tasks_state_distribution["failed"]
                ):
                    continue
                else:
                    state, _ = self.get_bulk_insert_state(
                        task_id, task_timeout, using, **kwargs
                    )
        ...
        if len(tasks_state_distribution["success"]) == len(task_ids):
            log.info(
                f"wait for bulk load tasks completed successfully, cost time: {end-start}"
            )
            return True, tasks_state
        else:
            log.info(
                f"wait for bulk load tasks completed failed, cost time: {end-start}"
            )
    ...
    def wait_all_pending_tasks_finished(self):
        ...
        while len(pending_tasks) > 0:
            ...
            log.debug(
                f"total pending tasks: {pending_task_ids} current pending tasks: {cur_pending_task_ids}"
            )
        ...
        for task in all_tasks:
            if task.task_id in pending_task_ids:
                log.info(
                    f"task {task.task_id} state transfer from pending to {task.state_name}"
                )
    ...
    def get_query_segment_info(
        ...
        func_name = sys._getframe().f_code.co_name
        res, is_succ = api_request(
            [self.ut.get_query_segment_info, collection_name, timeout, using]
        )
        check_result = ResponseChecker(
            res,
            func_name,
    ...
    def create_alias(
        ...
        func_name = sys._getframe().f_code.co_name
        res, is_succ = api_request(
            [self.ut.create_alias, collection_name, alias, timeout, using]
        )
        check_result = ResponseChecker(
            res,
            func_name,
    ...
    def drop_alias(
        self, alias, timeout=None, using="default", check_task=None, check_items=None
    ):
        timeout = TIMEOUT if timeout is None else timeout
        func_name = sys._getframe().f_code.co_name
    ...
    def alter_alias(
        ...
        func_name = sys._getframe().f_code.co_name
        res, is_succ = api_request(
            [self.ut.alter_alias, collection_name, alias, timeout, using]
        )
        check_result = ResponseChecker(
            res,
            func_name,
    ...
    def list_aliases(
        ...
        func_name = sys._getframe().f_code.co_name
        res, is_succ = api_request(
            [self.ut.list_aliases, collection_name, timeout, using]
        )
        check_result = ResponseChecker(
            res,
            func_name,
    ...
    def mkts_from_hybridts(self, hybridts, milliseconds=0.0, delta=None):
        res, _ = api_request(
            [self.ut.mkts_from_hybridts, hybridts, milliseconds, delta]
        )
        return res

    def create_user(
        self, user, password, using="default", check_task=None, check_items=None
    ):
        func_name = sys._getframe().f_code.co_name
        res, is_succ = api_request([self.ut.create_user, user, password, using])
        check_result = ResponseChecker(
            res, func_name, check_task, check_items, is_succ, using=using
        ).run()
        return res, check_result
    ...
    def reset_password(
        self, user, old_password, new_password, check_task=None, check_items=None
    ):
        func_name = sys._getframe().f_code.co_name
        res, is_succ = api_request(
            [self.ut.reset_password, user, old_password, new_password]
        )
        check_result = ResponseChecker(
            res, func_name, check_task, check_items, is_succ
        ).run()
        return res, check_result
    ...
    def update_password(
        ...
        func_name = sys._getframe().f_code.co_name
        res, is_succ = api_request(
            [self.ut.update_password, user, old_password, new_password]
        )
        check_result = ResponseChecker(
            res, func_name, check_task, check_items, is_succ
        ).run()
    ...
    def init_role(
        self, name, using="default", check_task=None, check_items=None, **kwargs
    ):
        func_name = sys._getframe().f_code.co_name
        res, is_succ = api_request([Role, name, using], **kwargs)
        self.role = res if is_succ else None
        check_result = ResponseChecker(
            res, func_name, check_task, check_items, is_succ, name=name, **kwargs
        ).run()
        return res, check_result
    ...
    def list_user(
        ...
        func_name = sys._getframe().f_code.co_name
        res, is_succ = api_request(
            [self.ut.list_user, username, include_role_info, using]
        )
        check_result = ResponseChecker(
            res, func_name, check_task, check_items, is_succ, using=using
        ).run()
    ...
    def role_remove_user(
        self, username: str, check_task=None, check_items=None, **kwargs
    ):
        func_name = sys._getframe().f_code.co_name
    ...
    def role_grant(
        ...
        **kwargs,
        ...
        res, check = api_request(
            [self.role.grant, object, object_name, privilege], **kwargs
        )
        check_result = ResponseChecker(
            res, func_name, check_task, check_items, check, **kwargs
        ).run()
    ...
    def role_revoke(
        ...
        **kwargs,
        ...
        func_name = sys._getframe().f_code.co_name
        res, check = api_request(
            [self.role.revoke, object, object_name, privilege], **kwargs
        )
        check_result = ResponseChecker(
            res, func_name, check_task, check_items, check, **kwargs
        ).run()
    ...
    def role_list_grant(
        self, object: str, object_name: str, check_task=None, check_items=None, **kwargs
    ):
        func_name = sys._getframe().f_code.co_name
