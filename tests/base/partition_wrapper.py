...
class ApiPartitionWrapper:
    ...
    def drop(self, check_task=None, check_items=None, **kwargs):
        ...
        check_result = ResponseChecker(
            res, func_name, check_task, check_items, succ, **kwargs
        ).run()
    ...
    def load(
        ...
        **kwargs,
        ...
        replica_number = (
            param_info.param_replica_num if replica_number is NaN else replica_number
        )

        func_name = sys._getframe().f_code.co_name
        res, succ = api_request(
            [self.partition.load, replica_number, timeout], **kwargs
        )
        check_result = ResponseChecker(
            res, func_name, check_task, check_items, is_succ=succ, **kwargs
        ).run()
        return res, check_result
    ...
    def insert(self, data, check_task=None, check_items=None, **kwargs):
        ...
        check_result = ResponseChecker(
            res, func_name, check_task, check_items, is_succ=succ, data=data, **kwargs
        ).run()
        return res, check_result
    ...
    def query(
        ...
        **kwargs,
        ...
        res, check = api_request(
            [self.partition.query, expr, output_fields, timeout], **kwargs
        )
        check_result = ResponseChecker(
            res,
            func_name,
    ...
    def delete(self, expr, check_task=None, check_items=None, **kwargs):
        ...
        check_result = ResponseChecker(
            res, func_name, check_task, check_items, is_succ=succ, expr=expr, **kwargs
        ).run()
        return res, check_result
