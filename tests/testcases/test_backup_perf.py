...
class TestPerf(TestcaseBase):
    ...
    def setup_perf(self, nb=1000):
        ...
        if self.prepare_data_done:
            log.info(
                f"*****************Test Perf Setup With nb {nb} Done, Skip*****************"
            )
            return
        else:
            log.info(
                f"*****************Test Perf Setup With nb {nb} Start*****************"
            )
        total_nb = nb
        cnt = 10
        coll_num = 1
    ...
    def restore_perf(self):
        ...
        t0 = time.perf_counter()
        res, result = BackupRestoreChecker(
            self.backup_name, "_bak", self.collections_to_backup
        ).run_task()
        t1 = time.perf_counter()
        log.info(f"create backup time: {t1 - t0} with {res}, {result}")
        return res, result
    ...
    def test_milvus_restore_backup_perf(self, benchmark, nb):
        self.setup_perf(nb=nb)
        res, result = benchmark.pedantic(
            self.restore_perf, setup=self.setup_perf, iterations=1, rounds=5
        )
        assert result is True
