...
class TestCreateBackup(TestcaseBase):
    ...
    def test_milvus_create_backup(
        self, collection_type, collection_need_to_backup, is_async
    ):
        # prepare data
        names_origin = []
    ...
    def test_milvus_create_backup_with_indexed_and_loaded(
        self,
        collection_type,
        collection_need_to_backup,
        is_async,
        collection_load_status,
    ):
        # prepare data
        names_origin = []
        back_up_name = cf.gen_unique_str(backup_prefix)
        if collection_type == "all":
            for is_binary in [True, False, False]:
                names_origin.append(cf.gen_unique_str(prefix))
                self.prepare_data(
                    names_origin[-1], is_binary=is_binary, check_function=True
                )
        if collection_type == "float":
            for is_binary in [False, False, False]:
                names_origin.append(cf.gen_unique_str(prefix))
        ...
        for name in names_origin:
            ...
            collection_info[name] = {"index_info": index_info, "load_state": loaded}
        ...
        for backup in backup_info:
            ...
            assert len(backup["index_infos"]) == len(
                collection_info[c_name]["index_info"]
            )
        res = self.client.list_backup()
        log.info(f"list backup response: {res}")
        if "data" in res:
